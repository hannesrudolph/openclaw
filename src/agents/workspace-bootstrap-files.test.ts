...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("includes MEMORY.md when present", async () => {
    ...
    expectSingleMemoryEntry(await loadWorkspaceBootstrapFiles(tempDir), "memory");
  });

  it("ignores lowercase memory.md when MEMORY.md is absent", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await writeWorkspaceFile({ dir: tempDir, name: "memory.md", content: "alt" });
    expect(getMemoryEntries(await loadWorkspaceBootstrapFiles(tempDir))).toHaveLength(0);
  });
...
describe("filterBootstrapFilesForSession", () => {
  ...
  it("returns all files for main session (no sessionKey)", () => {
    expect(filterBootstrapFilesForSession(mockFiles)).toStrictEqual(mockFiles);
  });

  it("returns all files for normal (non-subagent, non-cron) session key", () => {
    expect(filterBootstrapFilesForSession(mockFiles, "agent:default:chat:main")).toStrictEqual(
      mockFiles,
    );
  });

  it("filters to allowlist for subagent sessions", () => {
    expectSubagentAllowedBootstrapNames(
      filterBootstrapFilesForSession(mockFiles, "agent:default:subagent:task-1"),
    );
  });

  it("filters to allowlist for cron sessions", () => {
    expectCronAllowedBootstrapNames(
      filterBootstrapFilesForSession(mockFiles, "agent:default:cron:daily-check"),
    );
  });
});
